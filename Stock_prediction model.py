...
#column names relevant for analysis
num_col = ['Close', 'Open', 'High', 'Low', 'Volume']
#converting object columns to numeric values. NaN for data that is non-numeric
data[num_col] = data[num_col].apply(pd.to_numeric, errors='coerce')
#Filling NaN with mean value of the company stock
data[num_col] = data[num_col].fillna(data[num_col].mean())
#creates a new column- Target to indicate whether the stock price increased(1) or decreased(0) 
# in the next time step by comparing the actual price of the column and the shifted price
data['Target'] = (data['Close'].shift(-1) > data['Close']).astype(int)
#the last column is dropped as shift would generate an error
data = data[:-1]
